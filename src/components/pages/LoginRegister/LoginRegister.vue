...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      register1() {
        getData({ apiKey: 'register', data: { phone: Register_phone.value, password:  Register_password.value, password2: Register_password2.value, code: Register_code.value} })
          .then(data => {
            if (data.status == 'OK') {
              alert('注册成功，请重新登陆！');
              this.register = false;
              ...
              // var Time = 5;
              // this.Info = '注册成功，网页将在 '+Time+' 秒内跳转到登录界面';
              // var setVal = function() {
              //   this.register = false;
              //   this.login = true;
              //   this.Info = '';
