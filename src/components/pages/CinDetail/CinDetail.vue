  ...
  export default {
    ...
    mounted: function() {
      ...
      getData({ apiKey: 'cin_info', params: { cin_id: this.cin_id }  })
          .then(response => {
            ...
            // this.screens = ob["2018-8-1"];
            this.screens = [];
            for(var a in ob) {
              console.log('a' + a);
              for(var i = 0; i < ob[a].length; i++) {
                ob[a][i].starttime = new Date(ob[a][i].starttime).toLocaleString();
                console.log(ob[a][i]);
                this.screens.push(ob[a][i]);
    ...
    methods: {
          ...
          for(var a in ob) {
            console.log('a' + a);
            for(var i = 0; i < ob[a].length; i++) {
              var d = new Date(ob[a][i].starttime);
              ob[a][i].starttime = d.getFullYear() + '-' + (d.getMonth() + 1) + '-' + d.getDate() + ' ' + d.getHours() + ':' + d.getMinutes();
              console.log(ob[a][i]);
              this.screens.push(ob[a][i]);
              console.log(this.screens);
            }
